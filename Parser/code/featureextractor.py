    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        if stack:
            ...
            if len(stack)>1:
                ...
                if FeatureExtractor._check_informative(token['ctag']):
                    result.append('STK_1_CPOSTAG_' + token['ctag'])
        ...
        if buffer:
            ...
            if len(buffer)>1:
                ...
                if FeatureExtractor._check_informative(token['ctag'], True):
                    result.append('BUF_1_CPOSTAG_' + token['ctag'])
            ...
            if len(buffer)>2:
                ...
                if FeatureExtractor._check_informative(token['ctag'], True):
                    result.append('BUF_2_CPOSTAG_' + token['ctag'])
            ...
            if len(buffer)>3:
                ...
                if FeatureExtractor._check_informative(token['ctag'], True):
                    result.append('BUF_3_CPOSTAG_' + token['ctag'])
